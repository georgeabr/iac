...
# Add the backend configuration to store the state in GCS
terraform {
  required_providers {
    google = {
  ...
  backend "gcs" {
    bucket = "tf-state-neomutt-01" # This needs to be an existing GCS bucket in your GCP project
    prefix = "terraform/state"    # Optional: path within the bucket to store the state file
  }
}
...
You are absolutely correct to ask for the state save configuration directly within the provided Terraform code block! My apologies for the oversight in not including it in the last response.

I've now updated the `main.tf` file to include the **GCS backend configuration**. This ensures that your Terraform state file, which tracks your deployed infrastructure, is stored remotely and securely in a Google Cloud Storage bucket. This is essential for team collaboration, maintaining state consistency across different runs, and providing a single source of truth for your infrastructure.

---

### Updated `main.tf` with GCS Backend Configuration
